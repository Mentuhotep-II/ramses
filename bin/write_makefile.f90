subroutine output_makefile(filename)
  ...
  write(ilun,format)"# Do we want to test coverage?"
  write(ilun,format)"GCOV = 0"
  write(ilun,format)"# Compiler flavor: GNU or INTEL"
  write(ilun,format)"COMPILER = GNU"
  write(ilun,format)"# Target cluster, to set cluster specific optimisation flags (see Makefile.machines)"
  write(ilun,format)"MACHINE = generic"
  ...
  write(ilun,format)"NHYDRO = 2+$(NDIM)"
  write(ilun,format)"ifeq ($(SOLVER),mhd)"
  write(ilun,format)"   NHYDRO = 8"
  write(ilun,format)"endif"
  write(ilun,format)"NVAR := $(NHYDRO)+$(NENER)+$(NPSCAL)+$(NMETALS)"
  ...
  write(ilun,format)"BUILDCOMMAND =${MAKE} ${MAKEFLAGS}"
  write(ilun,format)"DEFINES = -DNVECTOR=$(NVECTOR) -DNDIM=$(NDIM) -DNPRE=$(NPRE) -DNENER=$(NENER) \"
  write(ilun,format)"          -DNVAR=$(NVAR) -DNHYDRO=$(NHYDRO) -DSOLVER$(SOLVER)"
  ...
  write(ilun,format)"endif"
  write(ilun,format)"#############################################################################"
  write(ilun,format)"# Compiler command for regular compilation (mpi or serial)"
  write(ilun,format)"F90 ="
  write(ilun,format)"# Serial compiler command and flags for helper files"
  write(ilun,format)"FC ="
  write(ilun,format)"# Base compiler flags for compilation"
  write(ilun,format)"FFLAGS_BASE ="
  write(ilun,format)"# Compiler flags for optimization"
  write(ilun,format)"FFLAGS_OPT = -O3"
  write(ilun,format)"# Compiler flags for linking"
  write(ilun,format)"LDFLAGS ="
  write(ilun,format)"# Load presets for MPIF90 and FFLAGS_OPT for specified machine"
  write(ilun,format)"-include clusters.mk"
  write(ilun,format)""
  write(ilun,format)"# GNU compiler (gfortran)"
  write(ilun,format)"ifeq ($(COMPILER),GNU)"
  write(ilun,format)"   # set compiler commands"
  write(ilun,format)"   ifeq ($(MPI),1)"
  write(ilun,format)"      F90 = $(MPIF90)"
  write(ilun,format)"   else"
  write(ilun,format)"      F90 = gfortran"
  write(ilun,format)"   endif"
  write(ilun,format)"   FC = gfortran -ffree-line-length-none -fimplicit-none"
  write(ilun,format)"   FFLAGS_BASE += -ffree-line-length-none -fimplicit-none"
  write(ilun,format)"   # add preprocessor macro definitions to compilation flags"
  write(ilun,format)"   FFLAGS_BASE += -cpp $(DEFINES)"
  write(ilun,format)"   ifeq ($(MPI),0)"
  write(ilun,format)"      FFLAGS_BASE += -DWITHOUTMPI"
  write(ilun,format)"   endif"
  write(ilun,format)"   # link-time optimization accross files"
  write(ilun,format)"   LDFLAGS += -flto -fwhole-program"
  write(ilun,format)"   FFLAGS_OPT += -flto -fwhole-program"
  write(ilun,format)"   # Flags for coverage calculation (resets optimization flags)"
  write(ilun,format)"   ifeq ($(GCOV),1)"
  write(ilun,format)"      FFLAGS_BASE += -fprofile-arcs -ftest-coverage"
  write(ilun,format)"      LDFLAGS = -fprofile-arcs -ftest-coverage"
  write(ilun,format)"      FFLAGS_OPT = -O0"
  write(ilun,format)"   else ifeq ($(DEBUG),1)"
  write(ilun,format)"      # Flags for debugging (resets optimization flags)"
  write(ilun,format)"      FFLAGS_BASE += -g -fbacktrace -fbounds-check -Wuninitialized -Wall"
  write(ilun,format)"      FFLAGS_BASE += -ffpe-trap=zero,underflow,overflow,invalid -finit-real=nan"
  write(ilun,format)"      LDFLAGS = -g"
  write(ilun,format)"      FFLAGS_OPT = -O0"
  write(ilun,format)"   endif"
  write(ilun,format)"endif"
  write(ilun,format)""
  ...
  write(ilun,format)"   endif"
  write(ilun,format)"   FC = ifort -fp-model source"
  write(ilun,format)"   FFLAGS_BASE += -fp-model source"
  write(ilun,format)"   # add preprocessor macro definitions to compilation flags"
  write(ilun,format)"   FFLAGS_BASE += -cpp $(DEFINES)"
  write(ilun,format)"   ifeq ($(MPI),1)"
  write(ilun,format)"      FFLAGS_BASE += -DNOSYSTEM"
  write(ilun,format)"   else"
  write(ilun,format)"      FFLAGS_BASE += -DWITHOUTMPI"
  write(ilun,format)"   endif"
  write(ilun,format)"   # link-time optimization and inlining accross files"
  write(ilun,format)"   LDFLAGS += -ipo -inline-forceinline"
  write(ilun,format)"   FFLAGS_OPT += -ipo -inline-forceinline"
  write(ilun,format)"   # Flags for debugging (resets optimization flags)"
  write(ilun,format)"   ifeq ($(DEBUG),1)"
  write(ilun,format)"      FFLAGS_BASE += -g -traceback -check bounds -warn all"
  write(ilun,format)"      FFLAGS_BASE += -fpe0 -ftrapuv -init=zero -init=snan -init=arrays"
  write(ilun,format)"      LDFLAGS = -g -traceback"
  write(ilun,format)"      FFLAGS_OPT = -O0"
  write(ilun,format)"   endif"
  write(ilun,format)"endif"
  write(ilun,format)""
  ...
  write(ilun,format)"MODOBJ = mpi_mod.o amr_parameters.o amr_commons.o amr_constants.o random.o pm_parameters.o sink_feedback_parameters.o \"
  write(ilun,format)"         pm_commons.o poisson_parameters.o collapse_parameters.o dump_utils.o constants.o file_module.o"
  write(ilun,format)"ifeq ($(GRACKLE),1)"
  ...
  write(ilun,format)"         amr_step.o update_time.o timer.o output_amr.o flag_utils.o \"
  ...
  write(ilun,format)"             multigrid_fine_commons.o multigrid_fine_fine.o \"
  ...
  write(ilun,format)"           courant_fine.o godunov_fine.o uplmde.o slope_types.o umuscl.o interpol_hydro.o \"
  ...
  write(ilun,format)"	$(F90) $(LDFLAGS) $(MODOBJ) $(AMRLIB) ramses.o -o $(EXEC)$(NDIM)d $(LIBS)"
  ...
  write(ilun,format)"	$(F90) $(LDFLAGS) $(MODOBJ) $(ATON_MODOBJ) $(AMRLIB) $(ATON_OBJ) ramses.o \"
  ...
  write(ilun,format)"		-DBUILDDATE=\'""$(BUILDDATE)""\' -DBUILDCOMMAND=\'""$(BUILDCOMMAND)""\' -c ../amr/write_gitinfo.f90 -o $@"
  ...
  write(ilun,format)"	$(F90) $(FFLAGS_BASE) $(FFLAGS_OPT) -c $^ -o $@ $(LIBS_OBJ) $(LIBS_OBJ_TURB)"
